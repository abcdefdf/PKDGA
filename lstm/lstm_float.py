...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        # self.emb = nn.Embedding(39, 32)
        self.lstm = nn.LSTM(5000, 32, num_layers=1, batch_first=True)
        self.lin = nn.Linear(32, 1)

        self.init_params()

    def init_hidden(self, batch_size):
        h = Variable(torch.zeros((1, batch_size, 32)))
        c = Variable(torch.zeros((1, batch_size, 32)))
        # if self.use_cuda:
        #     h, c = h.cuda(), c.cuda()
        return h, c
    ...
    def forward(self, x):
        ...
        output = output[:, -1, :]
        output = self.lin(output.contiguous().view(-1, 32))
